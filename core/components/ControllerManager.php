...
class ControllerManager extends BaseObject implements ComponentInterface
{
    ...
    private function registerChildController(){
        ...
        if (defined('DOING_AJAX') && DOING_AJAX) {
            $ctrlPos = ['credglv\extensions','credglv\front', 'credglv\admin'];
        } else {
            if (credglv()->wp->is_admin()) {
                $ctrlPos = ['credglv\admin'];
            } else {
                $ctrlPos =  ['credglv\extensions','credglv\front'];
            }
        }
        $cacheName = 'controller_maps_' . md5(serialize($ctrlPos)) . (credglv()->wp->is_admin() ? '_admin' : '_front');
        /** @var ControllerInterface[] $controllers */
        $controllers = [];
        if (empty($controllers) || CREDGLV_DEBUG) {
            $controllers = credglv()->helpers->file->findAllClass($ctrlPos, 'Controller',ControllerInterface::class);
        }
        if (defined('DOING_AJAX')) {
            $controllers = credglv()->hook->registerFilter('credglv_admin_controllers', $controllers);
            $controllers = credglv()->hook->registerFilter('credglv_front_controllers', $controllers);

        } else {
            $controllers = credglv()->hook->registerFilter('credglv' . (credglv()->wp->is_admin() ? '_admin' : '_front') . '_controllers', $controllers);
        }
        foreach ($controllers as $controller) {
            ...
            if (isset($params['actions']) && !empty($params['actions'])) {
                foreach ($params['actions'] as $action => $callable) {
                    ...
                    credglv()->wp->add_action($action, $callback, $priority, $paramCount);
                }
            }
            if (isset($params['ajax']) && !empty($params['ajax'])) {
                foreach ($params['ajax'] as $action => $callable) {
                    if (credglv()->wp->is_user_logged_in()) {
                        credglv()->wp->add_action('wp_ajax_' . $action, $callable);
                    } else {
                        credglv()->wp->add_action('wp_ajax_nopriv_' . $action, $callable);
                    }
                }
            }
        ...
        credglv()->wp->add_action('admin_menu', [$this, 'registerPage']);
        credglv()->hook->listenFilter('credglv_front_page', [$this, 'frontPages']);
