...
abstract class Controller extends BaseObject
{
    ...
    public function render($view = '', $data = [], $return = false) {
        ...
        $data['context'] = $this;
        $data['helpers'] = credglv()->helpers;
        if (file_exists($viewPath)) {
            $content = credglv()->helpers->general->renderPhpFile($viewPath, $data);
            credglv()->hook->registerHook("credglv_controller_{$this->getControllerName()}_render", $content);
            if ($return) {
                return $content;
            }
            print $content;
        } else {
            throw new RuntimeException( __("The view {$viewPath} could not be found", 'credglv'));
        }
    }
    ...
    public function renderFile($file, $data = [], $return = false) {
        if (file_exists($file)) {
            $data['context'] = $this;
            $content = credglv()->helpers->general->renderPhpFile($file, $data);
            credglv()->hook->registerHook("credglv_controller_{$this->getControllerName()}_render_file", $content);
            if ($return) {
                return $content;
            }
            print $content;
        } else {
            throw new RuntimeException( __("The view file {$file} could not be found", 'credglv'));
        }
    }
