...
class Mailer extends BaseObject implements ComponentInterface, MigrableInterface
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $this->mailTemplate = CREDGLV_PATH . DIRECTORY_SEPARATOR . 'templates' . DIRECTORY_SEPARATOR . 'emails';
    }
    ...
    public function init()
    {
        ...
        $this->mailers = [
            self::MAIL_STUDENT_ENROLL_FREE => [
                'title' => __('When student enrolled to a free course', 'credglv'),
                'supportedParams' => [
                    'STUDENT_NAME' => [
                        'label' => __('Student name', 'credglv'),
                    ...
                    'STUDENT_FIRST_NAME' => [
                        'label' => __('Student first name', 'credglv'),
                        'value' => function($student) {
                            /** @var Student $student */
                    ...
                    'STUDENT_PROFILE_URL' => [
                        'label' => __('Student profile URL', 'credglv'),
                        'value' => function($student) {
                            /** @var Student $student */
            ...
            self::MAIL_STUDENT_ENROLL_PAID => [
                'title' => __('When student paid and enrolled to course', 'credglv'),
                'supportedParams' => [
                    'STUDENT_NAME' => [
                        'label' => __('Student name', 'credglv'),
                        'value' => function($student) {
                    ...
                    'LIST_ITEMS' => [
                        'label' => __('List of course items', 'credglv'),
                        'value' => function ($order) {
                            /** @var OrderModel $order */
                            $template = credglv()->mailer->getDefaultTemplate(self::MAIL_STUDENT_ENROLL_LIST, true);
                            return credglv()->helpers->general->renderPhpFile($template, [
                               'order' => $order
                            ]);
                        }
                    ],
                    'ORDER_TOTAL' => [
                        'label' => __('Total amount of order', 'credglv'),
                        'value' => function($order) {
                            /** @var OrderModel $order */
                            return $order->total;
                        }
                    ...
                    'ORDER_SUBTOTAL' => [
                        'label' => __('Subtotal of order', 'credglv'),
                        'value' => function($order) {
                            /** @var OrderModel $order */
                    ...
                    'ORDER_TAX_CLASS' => [
                        'label' => __('Tax class of order', 'credglv'),
                        'value' => function($order) {
                            /** @var OrderModel $order */
            ...
            self::MAIL_INSTRUCTOR_COURSE_PUBLISH => [
                'title' => __('When instructor\'s course was published', 'credglv'),
                'supportedParams' => [
                    'INSTRUCTOR_NAME' => [
                        'label' => __('Instructor name', 'credglv'),
        ...
        $this->mailers = credglv()->hook->registerFilter('credglv_mailers', $this->mailers);
        credglv()->wp->do_action('credglv_email_ready', $this);
    }
    ...
    public function getMailTemplate($type) {
        $templates = credglv()->config->mail_templates;
        if (empty($templates)) {
            $templates = [];
        }
    ...
    {
        $mailTemplates =  credglv()->config->mail_templates;
        $mailTemplates[$type] = $template;
        credglv()->config->mail_templates = $mailTemplates;
    }
            ...
            foreach ($supportedParams as $name => $param) {
                ...
                foreach( $ref->getParameters() as $param) {
                    if (!isset($data[$param->name])) {
                        throw new RuntimeException(__("Error email template. Missing {$param->name} information", 'credglv'));
                    }
    ...
    public function onActivate()
    {
        ...
        $mailTemplates = credglv()->hook->registerFilter('credglv_default_mail_templates', $mailTemplates);
        credglv()->config->mail_templates = $mailTemplates;
    }
