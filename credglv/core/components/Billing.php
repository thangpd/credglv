...
EOF;
    ...
    public function init()
    {
        ...
        $this->attributes = credglv()->hook->registerFilter('credglv_billing_attributes', $this->attributes);
        foreach ($this->attributes as $attribute => $label) {
            $this->data[$attribute] = null;
        }
        $this->printableTemplate = credglv()->hook->registerFilter('credglv_billing_printable_template', $this->printableTemplate);
    }
    ...
    public static function loadByOrder($orderId){
        if (!is_object($orderId)) {
            if (is_numeric($orderId)) {
                $billObject = (object) credglv()->wp->get_post_meta($orderId, 'billing_address', true);
            } else {
                if (credglv()->helpers->general->canUnserialize($orderId)) {
    ...
    public static function loadByUser($userId)
    {
        ...
        /** @var Billing[] $bills */
        $bills = [];
        $orders = $wpdb->get_results( $wpdb->prepare("SELECT post_id FROM $wpdb->postmeta WHERE meta_key = %s AND meta_value = %s order by meta_id DESC" , 'credglv_order_user_id', $userId) );
        foreach ($orders as $meta) {
            $orderId = $meta->post_id;
            $billObject = credglv()->wp->get_post_meta($orderId, 'billing_address', true);
    ...
    {
        return credglv()->helpers->general->getPersonName($this->first_name, $this->last_name);
    }
    ...
    {
        if ($this->isEmpty()) {
            return __('No billing address found', 'credglv');
        } else {
