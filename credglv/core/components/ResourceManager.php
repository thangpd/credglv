...
use credglv\core\interfaces\StyleInterface;
use credglv\libs\CssMinifier;
...
class ResourceManager extends BaseObject implements ComponentInterface, ResourceManagerInterface, MigrableInterface {
	...
	public function init() {
		...
		try {
			credglv()->wp->add_action( 'wp_footer', [ $this, 'printInlineScript' ] );
			$pageUrls = credglv()->config->pages;
			$urls     = credglv()->config->getUrlConfigs();
			if ( class_exists( 'WooCommerce' ) ) {
				$urls['credglv_checkout'] = wc_get_checkout_url();
				$urls['credglv_cart']     = wc_get_cart_url();
			} else {
				foreach ( $urls as &$url ) {
					$url = credglv()->page->getPageUrl( $url );
				}
			}
			$preloader = apply_filters( 'credglv_preloader', [] );
			if ( ! empty( $preloader ) ) {
			...
			$this->jsConfigs = [
				'baseUrl'        => site_url(),
				'lazyShortcode'  => credglv()->config->lazyShortcode,
				'ajaxurl'        => admin_url( 'admin-ajax.php' ),
				'debug'          => CREDGLV_DEBUG ? true : false,
				'urls'           => $urls,
				'configs'        => [
					'autoSuccessVideo' => 70
				],
				/*'ui'             => [
					'preloader'         => '<div class="credglv-loader"></div>',
					'preloader_configs' => $preloader_configs,
				],*/
				'user_logged_in' => is_user_logged_in(),
				'user_id'        => get_current_user_id()
			];
			credglv()->hook->listenHook( 'wp_footer', [ $this, 'printFooterScripts' ] );
		} catch ( \Exception $e ) {
	...
	public function printFooterScripts() {
		if ( self::$footerPrinted == false ) {
			$data = apply_filters( 'credglv_shortcode_selfdo', [] );
			if ( isset( $data['credglv_do_shortcode'] ) ) {
				$str = 'jQuery(function(){credglv.shortcodes.do(' . json_encode( $data['credglv_do_shortcode'] ) . ');});';
				wp_add_inline_script( 'credglv.shortcode.footer', $str );
			}
...
EOF;
	...
	public function registerCoreResource() {
		$coreScripts = credglv()->config->resource['assets']['scripts'];
		$coreStyles  = credglv()->config->resource['assets']['styles'];
		...
		$this->jsConfigs = credglv()->hook->registerFilter( 'credglv_js_configs', $this->jsConfigs );
		$this->configScript( 'credglvConfig', $this->jsConfigs );

		do_action( Hook::CREDGLV_AFTER_CORE_RESOURCES, $coreScripts );
	...
	public function releaseScript( $scripts = [], $name = false ) {
		...
		$jsPath = CREDGLV_WR_DIR . DIRECTORY_SEPARATOR . $jsFile;
		...
		if ( file_exists( $jsPath ) ) {
			...
			$jsUrl  = site_url() . '/wp-content/uploads/credglv/' . $jsFile;
	...
	public function releaseStyle( $styles, $name = false ) {
		...
		$cssPath = CREDGLV_WR_DIR . DIRECTORY_SEPARATOR . $cssFile;
		...
		if ( file_exists( $cssPath ) ) {
			...
			$cssUrl  = site_url() . '/wp-content/uploads/credglv/' . $cssFile;
	...
	public function getKeyMap( $array ) {
		...
		if ( array_key_exists( $assetKey, $assetKeyMaps ) ) {
			...
			do {
				$assetKeyMap = credglv()->helpers->general->getRandomString( 6 );
	...
	private function convertUrlToPath( $url ) {
		$baseUrl = plugins_url( CREDGLV_NAME );

		return CREDGLV_PATH . DIRECTORY_SEPARATOR . ( str_replace( $baseUrl, '', $url ) );
	}
	 ...
	 * @param $script
	 */
	public function configScript( $name, $script, $handle = 'credglv' ) {
		credglv()->wp->wp_localize_script( $handle, $name, $script );
	}
	...
	public function registerShortcodeAssets( $post ) {
		$shortcodes = credglv()->shortcodeManager->getRegisteredShortcodes();

		foreach ( $shortcodes as $shortcode ) {
			if ( has_shortcode( $post->post_content, $shortcode->getId() ) ) {
				//$this->releaseShortcodeAssets($shortcode);
				wp_enqueue_style( 'credglv-shortcodes' );
				wp_enqueue_script( 'credglv-shortcodes' );
