...
use credglv\core\interfaces\CacheableControllerInterface;
...
class Page extends BaseObject implements ComponentInterface
{
     ...
     * @var string
     */
    private $credglvRootPage = '';
    /**
     * @var string
     */
    private $credglvRootPageSingle = '';
    ...
    public function __construct(array $config = [])
    {
        ...
        $this->credglvRootPage = CREDGLV_PATH . '/templates/page.php';
        $this->credglvRootPageSingle = CREDGLV_PATH . '/templates/single.php';
        credglv()->hook->listenHook('admin_menu', [$this, 'registerAdminPage']);
        credglv()->hook->listenFilter('template_include', [$this, 'registerFrontPage']);
        credglv()->hook->listenFilter('document_title_parts', [$this, 'changePageTitle']);
    }
    ...
    public function registerAdminPage()
    {
        ...
        $adminPages = credglv()->hook->registerFilter('credglv_admin_page', $adminPages);
    ...
    public function registerFrontPage($template)
    {
        ...
        $frontPages = credglv()->hook->registerFilter('credglv_front_page', $frontPages);
    ...
    public function execute()
    {
        if (!empty($this->pageHandler)) {
            ...
            if (!CREDGLV_DEBUG && $controller instanceof CacheableControllerInterface) {
                if (!empty($content)) {
                    return $content;
                }
            }
            $content = $controller->$actionName();
            $content = credglv()->helpers->general->minifyHtml($content);
            return $content;
        }
    }
    ...
    private function createPageTemplate($post_name, $template, $options = [])
    {
        ...
        if (count($page) == 0) {
            ...
            if ($postId) {
                update_post_meta($postId, 'credglv_page_template', urlencode(json_encode($template)));
            }
        }
    }
