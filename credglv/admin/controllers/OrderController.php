...
class OrderController extends AdminController implements ControllerInterface
{
    ...
    {
        global $post;
        credglv()->hook->listenFilter('manage_credglv_order_posts_columns', [$this, 'orderColumns']);
        credglv()->hook->listenFilter('post_row_actions', [$this, 'removeOrderActions']);
        $this->customColumns = credglv()->hook->registerFilter('credglv_order_list_columns', $this->customColumns);
        credglv()->hook->listenHook('manage_credglv_order_posts_custom_column', [$this, 'renderCustomColumns']);
        if (get_post_type() != 'credglv_order') return;
        parent::init(); // TODO: Change the autogenerated stub
//        credglv()->hook->listenHook(Hook::LEMA_AFTER_CORE_RESOURCES, [$this, 'registerAssets']);
        credglv()->hook->listenFilter('title_save_pre', [$this, 'autoOrderTitle']);
    ...
    {
        if( get_post_type() === 'credglv_order' ){
            unset( $actions['view'] );
    ...
    public function renderCustomColumns($column, $post_id = '')
    {
        ...
        if (in_array($column, array_keys($this->customColumns))) {
            switch ($column) {
                case 'student' :
                    $user = get_user_by('ID', $order->credglv_order_user_id);
                    if (!empty($user)) {
                        echo $user->display_name;
                    } else {
                        echo __('Guest', 'credglv');
                    }
                    break;
                case 'status' :
    ...
    {
        credglv()->wp->wp_enqueue_style('select2-style');
        credglv()->wp->wp_enqueue_script('select2');
    }
    ...
    {
        if ( 'credglv_order' == get_post_type() )
    ...
    public function addUser()
    {
        ...
        if (isset($_POST['order_id']) && $_POST['LemaOrder']) {
            ...
            if (!empty($credglvOrder['credglv_order_user_id'])) {
                credglv()->wp->update_post_meta($orderId, 'credglv_order_user_id', $credglvOrder['credglv_order_user_id']);
    ...
    public function addBillingAddress()
    {
        ...
        if (!empty($billing) && isset($_POST['order_id'])) {
            $orderId = $_POST['order_id'];
            credglv()->wp->update_post_meta($orderId, 'billing_address', serialize($billing));
            $this->order = OrderModel::findOne($orderId);
            $this->form = new Form();
            $this->form->bind($this->order);
            $response = [
    ...
    public static function registerAction()
    {
        ...
        return [
            ...
            'ajax' => [
                'credglv_order_add_user' => [self::getInstance(), 'addUser'],
                'credglv_order_add_item' => [self::getInstance(), 'addItem'],
                'credglv_order_remove_item' => [self::getInstance(), 'removeItem'],
